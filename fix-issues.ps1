...
# 2. 下载必要的模型
Write-Host "`n2. 下载必要的模型..." -ForegroundColor Yellow
cd ..
...
# 检查 HuggingFace 模型
Write-Host "检查 HuggingFace 模型..." -ForegroundColor Cyan
$huggingfacePath = ".\huggingface_cache"
if (-not (Test-Path $huggingfacePath)) {
    Write-Host "创建 HuggingFace 缓存目录..." -ForegroundColor Cyan
    New-Item -ItemType Directory -Path $huggingfacePath -Force | Out-Null
}
...
$token = (Invoke-RestMethod -Uri "http://localhost:8000/api/auth/login" -Method POST -Body '{"username":"admin","password":"123456"}' -ContentType "application/json").access_token
...
if ($token) {
    ...
    Write-Host "⚠️  无法获取token" -ForegroundColor Yellow
}
