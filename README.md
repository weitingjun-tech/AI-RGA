...
- **流式输出**：SSE 实时生成回答，首字延迟从十几秒降到 1 秒内
- **知识引用**：自动标注回答来源，提高可信度
- **多轮对话**：支持上下文连续对话
- **相关度过滤**：阈值过滤低相关度片段，抑制幻觉

### 🔬 可观测与评估
- **检索日志**：每次检索的双路候选、融合结果、耗时全量落库，
  可回答「答错了是没检索到，还是检索到了但模型没用」
- **黄金测试集**：30 条覆盖 fact / keyword / multihop / refusal 四类场景
- **检索指标**：Hit@K、Recall@K、MRR，支持对照实验验证优化是否真的有效

### 🔁 用户反馈闭环
- **点赞/点踩**：答案下方一键反馈，点踩可选原因标签（不准确/不完整/过时/无关）
- **Bad case 归档**：点踩记录自动关联当次检索明细，直接定位根因
- **正循环**：生产问题 → 检索日志归因 → 补知识库/改检索 → 进黄金测试集防回归
