...
Write-Host "       C盘清理工具 PowerShell版" -ForegroundColor Cyan
Write-Host "========================================"
Write-Host ""
...
if (Test-Path $doubaoCache) {
    Remove-Item $doubaoCache -Recurse -Force
    Write-Host "   已清理豆包浏览器缓存"
}
...
if (Test-Path $edgeCache) {
    Remove-Item $edgeCache -Recurse -Force
    Write-Host "   已清理Edge缓存"
}

# 7. 系统垃圾清理
Write-Host "`n[7/7] 清理系统垃圾" -ForegroundColor Yellow
$cleanCommand = "cleanmgr.exe /sagerun:1"
...
Write-Host "✓ NVIDIA缓存将在下次游戏运行时自动重建"
Write-Host "✓ 豆包需重启才能完全生效"
Write-Host "✓ 如需释放更多空间，可手动卸载不使用的软件"