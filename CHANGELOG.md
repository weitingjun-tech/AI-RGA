...
**15. 测试里的 Windows 路径断言在 Linux 上失败（CI 首次运行暴露）**
- **现象**：本地 Windows 上 227 个测试全绿；推到 GitHub 后 CI 的 pytest 任务变红。
  **这是本项目的第一次 CI 运行，第一次就抓到了问题**
- **根因**：`test_config.py` 里断言 `resolve_path("D:/data/x") == "D:/data/x"`，
  把 Windows 的**盘符语义**当成了通用事实。在 Linux 上 `D:/data/x` 没有前导 `/`，
  `Path.is_absolute()` 返回 `False`，于是被当成相对路径拼到 backend 目录下，
  实际得到 `/app/D:/data/x`
- **影响**：不是产品缺陷，是**测试自身的可移植性缺陷**——
  它会让人误以为代码有问题，或者更糟：让人开始习惯性忽略 CI 的红灯
- **修复**：把盘符断言移到 `@pytest.mark.skipif(os.name != "nt")` 的用例里；
  跨平台的那条只断言 `tmp_path`（它在任何平台上都是绝对路径）
- **验证方式**：用**运行中的 Linux 容器**跑一遍完整测试复现了该失败
  （镜像里就有全部依赖与代码），修复后再次在容器内跑通 ——
  比"推理一下 Linux 上应该会怎样"可靠得多
- **教训**：凡是涉及路径、编码、换行、大小写敏感的部分，
  **本地通过不代表跨平台通过**。这也正是加 CI 的意义
